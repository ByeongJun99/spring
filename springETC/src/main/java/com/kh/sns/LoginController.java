...
public class LoginController {
	...
	public String naverLoginCallback(HttpServletRequest request) {
		...
		String clientId = "17Cb2tkE7O78Ih042LE9";
		String clientSecret = "2gdEimMnpS";
		String code = request.getParameter("code");
		String state = request.getParameter("state");
		
		try {
			String redirectURI = URLEncoder.encode("http://localhost:8888/springETC/naver-login", "UTF-8");
			...
			if (responseCode == 200) {
				// 정상적으로 정보를 받아왔을때 result에 정보를 저장
...
//				System.out.println(totalObj.get("access_token"));
	...
	// API에 연결하기 위한 HttpURLConnection객체를 생성하고 반환하는 메서드
	...
	private static String readBody(InputStream body) {
		...
		try (BufferedReader br = new BufferedReader(streamReader)) {
			...
			throw new RuntimeException("API 응답을 읽는데에 실패하였습니다. : ", e);
		}
	}
}
